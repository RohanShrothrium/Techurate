...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
    ...
    return fabric_ca_client.register({enrollmentID: 'ICICI', affiliation: 'org1',role: 'client'}, admin_user);
}).then((secret) => {
    // next we need to enroll the user with CA server
    console.log('Successfully registered user - secret:'+ secret);

    return fabric_ca_client.enroll({enrollmentID: 'ICICI', enrollmentSecret: secret});
}).then((enrollment) => {
  console.log('Successfully enrolled member user "ICICI" ');
